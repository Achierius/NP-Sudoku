...
public:
  ...
  using cnf_clause_t              = std::vector<const std::pair<variable_t, bool> >;
  using cnf_variable_t            = std::pair<value_t, std::vector<std::shared_ptr<cnf_clause_t> > >;
  ...
  CNFEquation(const std::initializer_list<cnf_clause_t>& clauses);
  CNFEquation(const std::initializer_list<cnf_clause_t>& clauses,
  ...
  void addClause(const cnf_clause_t& new_clause);
  void removeClause(const cnf_clause_t& clause_to_delete);

  std::vector<std::shared_ptr<cnf_clause_t> > varClauses(variable_t variable);
...
private:
  ...
  std::list<std::shared_ptr<cnf_clause_t> > clauses_;
  std::array<cnf_variable_t, CNF_MAX> variables_;
